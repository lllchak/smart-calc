...
void write_op_token(token *curr_token, char operation,
                    size_t *curr_equation_idx, size_t shift) {
  /*
  Description:
...
void oshift(token *curr_token, char operation, size_t *num_tokens,
            size_t *curr_equation_idx, size_t shift) {
  /*
  Description:
...
bool is_function(const char *src, const char *func_name, size_t *shift) {
  ...
  */
  ...
  if (flag && src[length] != '(' && strcmp(func_name, "mod"))
    flag = false;
  *shift = length;
